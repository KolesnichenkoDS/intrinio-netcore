...
namespace Intrinio.Client
{
    ...
    public partial class ApiClient
    {
            ...
            String contentType)
        {
            ...
            foreach (var (key, param) in pathParams)
                request.AddParameter(key, param, ParameterType.UrlSegment);

            // add header parameter, if any
            foreach (var (key, param) in headerParams)
                request.AddHeader(key, param);

            // add query parameter, if any
            foreach (var (key, param) in queryParams)
                request.AddQueryParameter(key, param);

            // add form parameter, if any
            foreach (var (key, param) in formParams)
                request.AddParameter(key, param);

            // add file parameter, if any
            foreach (var (_, param) in fileParams)
            {
